...
```{r, fig.width=8.5}
p1 <- plot_frq(bns$q47, ylim=c(0,35), wrap.title = 40, wrap.labels = 15, 
               title="Did you apply for CalFresh EBT as an individual or with other people in your household?") +
  xlab('')
p2 <- plot_frq(bns$fed_through_EBT, ylim=c(0,15), wrap.title = 40, title="How many people in the household are regularly fed through your CalFresh EBT card?") + xlab('')
...
```{r, fig.width=7}
hist_with_box('q49_1', 10, 3, 0.2, NULL, c(0,12), 'Monthly Support ($)')
```
...
Looking at the above table, the general pattern seems to be that as household size increases so does the monthly average assistance from CalFresh. The household average was divided by the household size to find the "per person average" for CalFresh assistance. The overall monthly assistance average per-person is  `r tmp[7,3]`.

<br>

### Is This Enough?
*Note: for the first graph on if the allowance is enough we have `r print_n_reporting("q50")`, while for the second graph for when people run out we have `r print_n_reporting("q51")`*
```{r, fig.height=4.5, fig.width=8.5}
p1 <- plot_frq(bns$q50, title="Do you feel that this is enough money to provide healthy food for the month?") + xlab('')
p2 <- plot_frq(bns$q51, ylim=c(0,14), wrap.labels = 12, title="If \"No\", then when do you typically run out?") + xlab('')
...
```{r, fig.height=3.5}
plot_frq(bns$q54, ylim=c(0,20), wrap.labels = 15) + xlab('Food Pantry Use Frequency') + ylab("Frequency")
```
