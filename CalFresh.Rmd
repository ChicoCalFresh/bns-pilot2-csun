...
```
Of the participants in the survey `r unname(table(bns$q38)['Yes'])` claim to be U.S. citizens and `r unname(table(bns$q38)['No'])` claim to not be U.S. citizens. For those who did not claim U.S. citizenship, `r unname(table(bns$q39)['Yes'])` people claim to be lawful permanent residents while `r unname(table(bns$q39)['No'])` people do not claim to be lawful permanent residents. There are `r unname(table(bns$q40)['No'])` people who claim not to be approved for work study and `r unname(table(bns$q40)['Yes'])` who do claim to be approved for work study. There are `r unname(table(bns$q41)['No'])` students who do not receive Cal Grant A or B (consisting 12.4% of the data), and `r unname(table(bns$q41)['Yes'])` students who do receive Cal Grant A or B. The majority of people in the data (`r unname(table(bns$q44)['No'])` people) do not receive any meals from a campus meal plan. Further breakdown of the `r unname(table(bns$q44)['Yes'])` people that are on a campus meal plan claim the following assistance below:
...
plot_frq(bns$meals_per_week_mp, vjust=-0.2, ylim = c(0,2.5)) + 
...
Of people who receive a campus meal plan, `r unname(table(bns$meals_per_week_mp)['11+'])` people receive more than 11 meals per week , while `r unname(table(bns$meals_per_week_mp)['< 11'])` person receives less than 11 meals per week. 
...
<div class = "col-md-6">
```{r, fig.height=3, fig.width=4}
plot_frq(bns$calfresh_eligible, title = "Most Likely to be CalFresh Eligible") + 
...
We can see that the majority of participants are most likely eligible for CalFresh (`r unname(table(bns$calfresh_eligible)['Yes'])` people), while `r unname(table(bns$calfresh_eligible)['No'])` people are most likely not eligible for these benefits.
</div>
</div>
