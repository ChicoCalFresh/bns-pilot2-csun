...
The following questions were asked in regard to students' sleep patterns. Questions included: their average nightly sleep during the academic year (on weekdays and weekends), if they feel sleep deprived, and what the causes of their sleep deprivation come from. They were also asked if they are tired at work/school, how often they are tired there, and if they have fallen asleep in class in the past year. 

<br>

## Academic Year
**On average, during the academic year, how many hours of sleep do you get in a typical 24-hour period?** *Note that for the week days we have `r print_n_reporting("q84")`, while for the weekends we have `r print_n_reporting("q85")`*

#### During the Week (Monday-Friday)
```{r, fig.height=3.5, fig.width=7}
hist_with_box('q84', 45, 8, 0.4, NULL, c(0,55), 'Number of Hours')
```
...
```{r}
hist_with_box('q86', 22, 4, 0.4, NULL, c(0,25), 'Days Per Month')
```
...
The data indicates the most common response of how often people felt tired at work or school was "`r tolower(names(sort(table(bns$q88), decreasing = TRUE))[1])`" (`r unname(sort(table(bns$q88), decreasing = TRUE))[1]` people). There were `r unname(sort(table(bns$q88), decreasing = TRUE))[2]` people that claimed they were `r tolower(names(sort(table(bns$q88), decreasing = TRUE))[2])` tired, `r unname(sort(table(bns$q88), decreasing = TRUE))[3]` people claimed that they were `r tolower(names(sort(table(bns$q88), decreasing = TRUE))[3])` tired, and `r unname(sort(table(bns$q88), decreasing = TRUE))[4]` person claimed that they were `r tolower(names(sort(table(bns$q88), decreasing = TRUE))[4])` tired.
</div>
</div>
...
plot_frq(bns$q89, ylim=c(0,75)) + xlab('') + ylab("Frequency")
```
</div>
...
The data indicates that `r unname(sort(table(bns$q89), decreasing = TRUE)['No'])` people claim that they have not fallen asleep in class in the past year, while `r unname(sort(table(bns$q89), decreasing = TRUE)['Yes'])` people claim that they have fallen asleep in class.
</div>
</div>
